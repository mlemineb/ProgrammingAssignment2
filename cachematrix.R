...
#This function allow us to create a cacheable matrix
makeCacheMatrix <- function(x = matrix()) {
 
   inverse<- NULL                                   # This variable will hold the inverse matrix, but for now is being set to null,                                                 # since the inverse of the matrix as not been calculated yet
  set <- function(y) {                              # This function set the value of vector
    x <<- y                                         # update the old matrix to the new matrix
    inv <<- NULL                                # reset the inverse of the matrix of the new matrix.
  }
...
# This function computes the inverse of the special "matrix" created with the function "makeCacheMatrix". 
# The first step its do ,is to checks if the inverse of the matrix has already been calculated. 
